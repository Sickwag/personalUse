...
template <typename T>
static T get_value_by_key(const json::value& json, const std::string& key) {
...
Config read_config(const std::string& config_path) {
    ...
    Config cfg;
    cfg.exclude = transfor_glob_to_reg(get_value_by_key<std::vector<std::string>>(j, "exclude"));   // UNTEST regex func
    cfg.display = generate_bitmask<Display_column>(get_value_by_key<json::object>(j, "display"));   // UNSURE binary value is right?
    cfg.output_type = generate_bitmask<Output_type>(get_value_by_key<json::object>(j, "output_type"));
    cfg.sort_method = generate_bitmask<Sort_method>(get_value_by_key<json::object>(j, "sort_method"));
    return cfg;
}
